...
type FQDNDataClient interface {
	...
	SubscribeToDNSPolicies(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[DNSPolicyResponse, DNSPolicy], error)
...
func (c *fQDNDataClient) SubscribeToDNSPolicies(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[DNSPolicyResponse, DNSPolicy], error) {
	...
	x := &grpc.GenericClientStream[DNSPolicyResponse, DNSPolicy]{ClientStream: stream}
	return x, nil
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type FQDNData_SubscribeToDNSPoliciesClient = grpc.BidiStreamingClient[DNSPolicyResponse, DNSPolicy]
...
type FQDNDataServer interface {
	...
	// In case of any client side error, cilium agent will cancel the stream and SDP will have to re-subscribe.
	// In case of any server side error, cilium agent will send an error response and SDP will have to re-subscribe.
	SubscribeToDNSPolicies(grpc.BidiStreamingServer[DNSPolicyResponse, DNSPolicy]) error
	// UpdateMappingRequest is used by the Standalone DNS proxy to update ciliium agent with FQDN-IP mappings which in turn update L3/L4 policy maps.
	// In case of any error, SDP will either retry the connection if the error is server side or will error out.
	// Note: In case of concurrent updates, since this is called in a callback(notifyDNSMsg) from the DNS server it follows the same behavior as
	// the inbuilt dns proxy in cilium.
...
func (UnimplementedFQDNDataServer) SubscribeToDNSPolicies(grpc.BidiStreamingServer[DNSPolicyResponse, DNSPolicy]) error {
...
func _FQDNData_SubscribeToDNSPolicies_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(FQDNDataServer).SubscribeToDNSPolicies(&grpc.GenericServerStream[DNSPolicyResponse, DNSPolicy]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type FQDNData_SubscribeToDNSPoliciesServer = grpc.BidiStreamingServer[DNSPolicyResponse, DNSPolicy]
